...
but it carries usernames from one IdP to the next, so it needs improvement to only carry cognito (your) user pools names around.
    ...
    - Facebook FBSDKApplicationDelegate bug.  ApplicationDidFinishLaunching with options ALWAYS returns NO, it needs to return Yes, or the sessions get 
torn down.
