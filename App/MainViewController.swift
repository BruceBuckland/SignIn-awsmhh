...
class MainViewController: UITableViewController {
    ...
    func recordIdentityUser() {
        let identityManager = AWSIdentityManager.defaultIdentityManager()
        let syncClient: AWSCognito = AWSCognito.defaultCognito()
        let identityUsers: AWSCognitoDataset = syncClient.openOrCreateDataset("identity_users")
        identityUsers.setString("\(identityManager.userName!)", forKey: "\(identityManager.providerKey(identityManager.currentSignInProvider as! AWSSignInProvider)).userName")
        identityUsers.setString("providerKey", forKey: "\(identityManager.providerKey(identityManager.currentSignInProvider as! AWSSignInProvider))")
        
        identityUsers.synchronize().continueWithExceptionCheckingBlock({(result: AnyObject?, error: NSError?) -> Void in
            if let error = error {
                print("identityUsers AWS task error: \(error.localizedDescription)")
            } else {
                NSLog("Saved identity_users")
            }
        })
    }
